...
namespace kt::Utils {
	struct CollisionData {
		const sf::Vector2f collisionNormalDirection{};
		const float overlapMagnitude{};
	};
	...
	std::optional<CollisionData> detectCollision(const kt::Shapes::RectangleEntity&, const kt::Shapes::CircleEntity&);
}